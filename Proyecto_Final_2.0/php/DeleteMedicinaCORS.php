...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id === null) {
        ...
        echo json_encode(array('message' => 'No se proporcionó el ID del Medicina.'));
        exit();
    }

    // Configuración de la base de datos SQLite (puedes ajustarla según tu configuración)
    $dbFile = 'vetnegris.sqlite3'; // Nombre del archivo SQLite
    ...
    $sql = "DELETE FROM Medicina WHERE id = :id";
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo json_encode(array('message' => 'Medicina eliminado correctamente'));
        } else {
            // Si no se encontró el distribuidor con el ID proporcionado, responder con un mensaje de error
            http_response_code(404); // Not Found
            echo json_encode(array('message' => 'No se encontró el Medicina con el ID proporcionado'));
        }
    } catch (PDOException $e) {
        // Si hay un error en la ejecución de la consulta, responder con un mensaje de error
        http_response_code(500); // Internal Server Error
        echo json_encode(array('message' => 'Error al eliminar el Medicina: ' . $e->getMessage()));
    }
} else {
