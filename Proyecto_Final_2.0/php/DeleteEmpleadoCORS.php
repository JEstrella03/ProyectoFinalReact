...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id === null) {
        ...
        echo json_encode(array('message' => 'No se proporcionó el ID del empleado.'));
        exit();
    }
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo json_encode(array('message' => 'No se encontró el empleado con el ID proporcionado'));
        }
    } catch (PDOException $e) {
        // Si hay un error en la ejecución de la consulta, responder con un mensaje de error
        http_response_code(500); // Internal Server Error
        echo json_encode(array('message' => 'Error al eliminar el empleado: ' . $e->getMessage()));
    }
} else {
