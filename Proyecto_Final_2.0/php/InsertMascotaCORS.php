...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['nombre']) || empty($data['edad']) || empty($data['estado']) || empty($data['dueno'])) {
        http_response_code(400); // Bad Request
        echo json_encode(array('message' => 'Faltan datos requeridos para insertar el mascota'));
        exit();
    }

    // Preparar los datos del mascota
    $mascota = [
        'nombre' => $data['nombre'],
        'edad' => $data['edad'],
        'estado' => $data['estado'],
    ...
    try {
        // Preparar la consulta SQL para insertar el mascota
        $query = 'INSERT INTO Maskota (nombre, edad, estado, dueno) VALUES (:nombre, :edad, :estado, :dueno)';
        $statement = $conex->prepare($query);

        // Ejecutar la consulta con los datos del mascota
        $result = $statement->execute($mascota);

        if ($result) {
            $id = $conex->lastInsertId(); // Obtener el ID del mascota insertado
            http_response_code(201); // Código 201: Created
            echo json_encode(array('message' => 'mascota insertada correctamente', 'id' => $id));
        } else {
            http_response_code(400); // Código 400: Bad Request
            echo json_encode(array('message' => 'Error al intentar insertar la mascota'));
        }
